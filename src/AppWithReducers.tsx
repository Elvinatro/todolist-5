...
import {changeTaskStatusAC, createTasksAC, deleteTasksAC, editTaskSpanAC, tasksReducer} from "./model/tasks-reducer.ts";
...
export const AppWithReducers = () => {
    ...
    const [todoList, dispatchTodo] = useReducer(todoListsReducer, [
    ...
    const createTask = (todoListId: string, title: string) => {
        dispatchTasks(createTasksAC(todoListId, title))
    }
    ...
    const EditTaskSpan = (todoListId: string, taskId: string, newValue: string) => {
        dispatchTasks(editTaskSpanAC(todoListId, taskId, newValue))
    }

    //-----------------------------------------------TodoList---------------------------------------------------------//

    const removeTodoList = (todoListId: string) => {
        dispatchTodo(removeTodoAC(todoListId))
        dispatchTasks(removeTodoAC(todoListId))
    }

    const addTodoList = (value: string) => {
        const todoListId = v1()
        dispatchTodo(addTodoAC(todoListId, value))
        dispatchTasks(addTodoAC(todoListId, value))
    }

    const changeTodoFilter = (todoListId: string, filter: FilterValues) => {
        dispatchTodo(changeTodoFilterAC(todoListId, filter))
    }

    const editTodoTitle = (todoListId: string, value: string) => {
        dispatchTodo(editTodoTitleAC(todoListId, value))
    }
